...
public:
  ...
  virtual const int getFlightData(byte){return 0;};
...
public:
...
#ifdef DEBUG_I2C
    Serial.print("Looking for Accel BMA180 ...  ");
#endif
    if (readWhoI2C(accelAddress) != 0x03) {// page 52 of datasheet
      Serial.println("Accelerometer not found!");
#ifdef DEBUG_I2C
    } else {
	  Serial.println("Accel BMA180 found!");
#endif
    }
  ...
  void measure(void) {
    ...
    for (byte axis = XAXIS; axis < LASTAXIS; axis++) {
      if (axis == XAXIS) {
        accelADC[axis] = (readReverseShortI2C() >> 2) - accelZero[axis];
      } else {
        accelADC[axis] = accelZero[axis] - (readReverseShortI2C() >> 2);
      }
      //accelData[axis] = computeFirstOrder(accelADC[axis] * accelScaleFactor, &firstOrder[axis]);
      accelData[axis] = filterSmooth(accelADC[axis] * accelScaleFactor, accelData[axis], smoothFactor);
    }
  ...
  void calibrate(void) {
    ...
    for (byte calAxis = XAXIS; calAxis < ZAXIS; calAxis++) {
      ...
      for (int i=0; i<FINDZERO; i++) {
        ...
        findZero[i] = readReverseShortI2C(accelAddress) >> 2; // last two bits are not part of measurement
        delay(10);
      }
      accelZero[calAxis] = findMedian(findZero, FINDZERO);
    }
...
public:
  ...
  void measure(void) {
    ...
    for (byte axis = XAXIS; axis < LASTAXIS; axis++) {
      if (axis == XAXIS)
        accelADC[axis] = readReverseShortI2C() - accelZero[axis];
      else
        accelADC[axis] = accelZero[axis] - readReverseShortI2C();
      //accelData[axis] = computeFirstOrder(accelADC[axis] * accelScaleFactor, &firstOrder[axis]);
      accelData[axis] = filterSmooth(accelADC[axis] * accelScaleFactor, accelData[axis], smoothFactor);
    }
  }
