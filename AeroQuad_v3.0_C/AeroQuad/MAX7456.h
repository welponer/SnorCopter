...
void displayVoltage() {
  ...
  writeChars( buf, 6, ((armed == ON) && (batteryStatus != BATTERY_MOBITOR_OK)) ? 1 : 0, VOLTAGE_ROW, VOLTAGE_COL );    
}
#endif
...
void displayArtificialHorizon() {
  ...
  short AIrows[5]   = {
    0,0,0,0,0  };  //Holds the row, in pixels, of AI elements: pitch then roll from left to right.
...
byte displayNotify(){
  ...
  if (_n_flags & OSD_NOW) {
    ...
    if ((_n_flags&OSD_CURSOR) && (_n_cstart!=255)) {
      ...
      if (_n_cstart > 0) {
        writeChars(_n_buf, _n_cstart, ((_n_flags&OSD_INVERT) ? 2 : 0) | ((_n_flags & OSD_BLINK) ? 1 : 0), NOTIFY_ROW,NOTIFY_COL);
      }

      writeChars(_n_buf+_n_cstart, _n_cstop-_n_cstart+1, ((_n_flags & OSD_INVERT) ? 2 : 0) | ((_n_flags & OSD_BLINK) ? 0 : 1), NOTIFY_ROW,NOTIFY_COL + _n_cstart);

      if (_n_cstop<27) {
        writeChars(_n_buf+_n_cstop+1, 27-_n_cstop, ((_n_flags&OSD_INVERT) ? 2 : 0) | ((_n_flags & OSD_BLINK) ? 1 : 0), NOTIFY_ROW,NOTIFY_COL + _n_cstop + 1);
      }
    }
    else {
      writeChars(_n_buf,28, ((_n_flags & OSD_INVERT) ? 2 : 0) | ((_n_flags & OSD_BLINK) ? 1 : 0), NOTIFY_ROW,NOTIFY_COL);
    }
    return 1;
  }
